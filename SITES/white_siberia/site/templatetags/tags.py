...
def load_dealers(request):
    ...
    cities = {city[0]: [] for city in Dealer.city_choices}

    dealers = Dealer.objects.select_related('address').all().order_by('position')
    for dealer in dealers:
        if not dealer.city in cities:
            continue
        coords = []
        if dealer.address:
        ...
        cities[dealer.city].append(obj)
    result = []
    for city in Dealer.city_choices:
        result.append({
            'id': city[0],
            'name': city[1],
            'dealers': cities[city[0]],
        })
