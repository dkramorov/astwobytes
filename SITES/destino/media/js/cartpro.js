		...
		try {
			switch (type) {
				...
				case 'url':
					...
					url = url.replace('form_key', 'formKey');
					new Ajax.Request(url, {
						method: 'post',
						postBody: "form_key="+formKeyJS,
						onCreate: function() {
							_this.showLoading();
						},
...
var setLocation = (function() {
	return function(url) {
		if (typeof enable_ajax_cart !== 'undefined' &&  enable_ajax_cart && url.search('checkout/cart/add') != -1)
			...
			if (typeof enable_ajax_cart !== 'undefined' &&  enable_ajax_cart && typeof cartpro_baseurl !== 'undefined'  && url !== cartpro_baseurl && url.search('blog') == -1 && url.search('catalog/product_compare/index/items') === -1) {
