...
# template = например, web/html_paginator.html
# 1 = личный кабинет шаблон
#################################################
def navigator(paginator, q_string, template=None):
  ...
  ends = analyze_digit(paginator['total_records'], end=(u"запись", u"записей", u"записи"))
  ...
  for i in range(start_page, end_page):
    ...
    if i == start_page:
      if paginator['cur_page'] == 1:
        ...
        # --------------------------------------------------
        # Если нужно пропускать page, by на первую страничку
        # это должно быть в q_string['pass_first_page'] = 1
        # --------------------------------------------------
        href = "%s" % q_string['link']
        if page_request:
          href += "?%s" % page_request
          if not "pass_first_page" in q_string:
